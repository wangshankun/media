...
NvJpeg::NvJpeg(int width = 1920, int height = 1080, int device_id = 0, CUcontext* ffmpeg_ctx = NULL, AVFrame * ff_gpu_frame = NULL, nvjpegOutputFormat_t fmt = NVJPEG_OUTPUT_YUV)
{
    ...
    //cuCtxPushCurrent(*ffmpeg_ctx);
    //cuCtxSetCurrent(*ffmpeg_ctx);
...
NvJpeg::~NvJpeg()
{
    ...
    checkCudaErrors(cudaFree(_pBuffer));
}

int NvJpeg::decompress_to_gpubuf(const unsigned char* jpgBuffer, const uint32_t jpgSize)
{
